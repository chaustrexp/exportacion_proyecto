...
// Los datos ($registros, $fichas, etc.) ya estÃ¡n disponibles en el scope
?>
...
<div class="main-content">
    ...
    <div style="padding: 32px 32px 24px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #e5e7eb;">
        <div style="flex: 1;">
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 4px;">
                ...
                <?php
                // Verificar si el controlador estÃ¡ disponible
        ...
        <a href="<?php echo BASE_PATH; ?>asignacion/crear" class="btn btn-primary" style="display: inline-flex; align-items: center; gap: 8px; text-decoration: none;">
    ...
    <?php if (isset($_GET['msg'])): ?>
        <div class="alert alert-success" style="margin: 24px 32px;">
            <?php 
            if ($_GET['msg'] == 'creado') echo 'âœ“ AsignaciÃ³n creada exitosamente';
            if ($_GET['msg'] == 'actualizado') echo 'âœ“ AsignaciÃ³n actualizada exitosamente';
            if ($_GET['msg'] == 'eliminado') echo 'âœ“ AsignaciÃ³n eliminada exitosamente';
            ?>
        </div>
    <?php endif; ?>
        ...
        <div style="background: white; padding: 20px; border-radius: 12px; border: 1px solid #e5e7eb; display: flex; align-items: center; gap: 20px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
            <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <div>
                    ...
                    <p style="margin: 0; font-size: 11px; color: #6b7280;">VisualizaciÃ³n rÃ¡pida de asignaciones</p>
                </div>
            </div>
            <div style="flex: 1;">
                <select id="filtro-maestro-programa" onchange="sincronizarFiltroPrograma(this.value)" style="width: 100%; padding: 10px 16px; border: 1px solid #e5e7eb; border-radius: 8px; font-size: 14px; font-weight: 600; color: #374151; cursor: pointer; background: #f9fafb;">
                    <option value="">Mostrar Todos los Programas de FormaciÃ³n</option>
                    <?php
                    $programas = array_unique(array_column($registros, 'programa_nombre'));
                    sort($programas);
                    foreach ($programas as $prog):
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="7" style="text-align: center; padding: 60px 20px; color: #6b7280;">
                            ...
                            <a href="<?php echo BASE_PATH; ?>asignacion/crear" class="btn btn-primary btn-sm">Crear Primera AsignaciÃ³n</a>
                        </td>
                    </tr>
                    <?php else: ?>
...
<script>
    // FunciÃ³n para sincronizar el filtro de programa maestro con el filtro de programa detallado
        ...
        if (form) {
            ...
            inputs.forEach(input => {
                input.addEventListener('input', function() {
                    ...
                    // Sincronizar hacia atrÃ¡s el filtro maestro si el cambio viene del detallado
    ...
    // FunciÃ³n especial para filtrar asignaciones con fechas
            ...
            if (dayAssignments.length > 0) {
                dayAssignments.forEach(asig => {
                    ...
                    calendarHTML += `<div style="background: #39A900; color: white; padding: 6px 8px; border-radius: 4px; font-size: 11px; margin-bottom: 4px; cursor: pointer;" onclick="event.stopPropagation(); verDetalleAsignacion(${asigId})" title="${asig.instructor_nombre}">
                        <div style="font-weight: 600;">ðŸ“š ${asig.ficha_numero}</div>
        ...
        if (dayAssignments.length === 0) {
            calendarHTML += `
                <div style="text-align: center; padding: 60px 20px; color: #6b7280;">
                    ...
                    <p style="margin: 0 0 16px; font-size: 18px; font-weight: 600;">No hay asignaciones para este dÃ­a</p>
                    <a href="${window.BASE_PATH}asignacion/crear" class="btn btn-primary" style="text-decoration: none; display: inline-flex; align-items: center; gap: 8px;">
                    ...
                    calendarHTML += `
                        <div style="background: white; border: 2px solid #e5e7eb; border-left: 4px solid #39A900; border-radius: 8px; padding: 20px; margin-bottom: 12px; transition: all 0.2s; cursor: pointer;" 
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px;">
                                <div>
                                    <div style="font-weight: 700; color: #1f2937; font-size: 18px; margin-bottom: 4px;">ðŸ“š Ficha ${asig.ficha_numero}</div>
    ...
    // FunciÃ³n para ver asignaciones de un dÃ­a
        ...
        if (asignaciones.length === 0) {
            asignacionesHTML = `
                <div style="text-align: center; padding: 40px 20px; color: #6b7280;">
                    <div style="font-size: 48px; margin-bottom: 16px;">ðŸ“…</div>
                    <p style="margin: 0 0 16px; font-size: 16px;">No hay asignaciones para este dÃ­a</p>
                    <a href="${window.BASE_PATH}asignacion/crear" class="btn btn-primary btn-sm" style="text-decoration: none;">
                        Crear AsignaciÃ³n
                    </a>
                </div>
            `;
            ...
            asignacionesHTML = asignaciones.map(asig => {
                ...
                <div style="background: white; border: 2px solid #e5e7eb; border-radius: 8px; padding: 16px; margin-bottom: 12px; transition: all 0.2s; cursor: pointer;" 
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                        <div style="font-weight: 700; color: #1f2937; font-size: 16px;">ðŸ“š Ficha ${asig.ficha_numero}</div>
            ...
            <div id="modalDia" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.6); display: flex; align-items: center; justify-content: center; z-index: 9999; padding: 20px;" onclick="if(event.target.id==='modalDia') this.remove()">
                <div style="background: white; border-radius: 12px; max-width: 600px; width: 100%; box-shadow: 0 25px 70px rgba(0,0,0,0.4); overflow: hidden; max-height: 90vh; overflow-y: auto;" onclick="event.stopPropagation()">
                    ...
                    <div style="background: linear-gradient(135deg, #39A900 0%, #007832 100%); padding: 24px; color: white;">
                        <h3 style="font-size: 22px; font-weight: 700; margin: 0 0 4px;">Asignaciones del DÃ­a</h3>
                    ...
                    <div style="padding: 16px 24px; border-top: 1px solid #e5e7eb; display: flex; justify-content: space-between; gap: 12px;">
                        ...
                        <a href="${window.BASE_PATH}asignacion/crear" class="btn btn-primary" style="text-decoration: none;">Nueva AsignaciÃ³n</a>
                    </div>
                </div>
            </div>
        `;
    ...
    // FunciÃ³n para ver detalle de una asignaciÃ³n
    function verDetalleAsignacion(id) {
        fetch(`${window.BASE_PATH}views/asignacion/get_asignacion.php?id=${id}`)
                ...
                const modal = `
                    <div id="modalDetalle" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.6); display: flex; align-items: center; justify-content: center; z-index: 10000; padding: 20px;" onclick="if(event.target.id==='modalDetalle') this.remove()">
                        <div style="background: white; border-radius: 12px; max-width: 600px; width: 100%; box-shadow: 0 25px 70px rgba(0,0,0,0.4); overflow: hidden;" onclick="event.stopPropagation()">
                            ...
                            <!-- Header -->
                            <div style="background: linear-gradient(135deg, #39A900 0%, #007832 100%); padding: 24px; color: white;">
                                <h3 style="font-size: 22px; font-weight: 700; margin: 0 0 4px;">Detalle de AsignaciÃ³n</h3>
                            ...
                            <div style="padding: 24px;">
                                ...
                                <div style="background: ${data.estado_bg}; border-left: 4px solid ${data.estado_color}; padding: 16px; border-radius: 8px; margin-bottom: 24px;">
                                    <div style="font-weight: 700; color: ${data.estado_color}; font-size: 16px; margin-bottom: 4px;">
                                        ${data.estado === 'Activa' ? 'âœ“' : data.estado === 'Pendiente' ? 'â³' : 'âœ•'} ${data.estado}
                                    </div>
                                ...
                                <div style="display: grid; gap: 16px;">
                                    ...
                                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                                        <div>
                                            ...
                                            <div style="font-size: 16px; font-weight: 600; color: #1f2937;">â° ${data.hora_inicio}</div>
                                        </div>
                                        <div>
                                            <div style="font-size: 12px; color: #6b7280; margin-bottom: 4px; text-transform: uppercase; font-weight: 600;">Hora Fin</div>
                                            <div style="font-size: 16px; font-weight: 600; color: #1f2937;">â° ${data.hora_fin}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            .catch(error => {
                ...
                alert('Error al cargar los detalles de la asignaciÃ³n');
            });
    }

// LÃ³gica para detectar si se debe mostrar la pestaÃ±a de calendario al cargar
document.addEventListener('DOMContentLoaded', function() {
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('tab') === 'calendar') {
        showTab('calendar');
        
        // Limpiar el parÃ¡metro de la URL sin recargar para que no persista si el usuario recarga manualmente
