    ...
    <div style="max-width: 700px; margin: 0 auto; padding: 32px;">
        <div style="background: white; border-radius: 12px; border: 1px solid #e5e7eb; padding: 32px;">
            ...
            <form method="POST" action="<?php echo BASE_PATH; ?>instructor/editar?id=<?php echo htmlspecialchars($registro['inst_id'] ?? ''); ?>">
                ...
                <!-- CU-01: Estado del instructor — campo requerido para validación -->
                <div class="form-group">
                    <label style="display: block; font-size: 14px; font-weight: 600; color: #374151; margin-bottom: 8px;">
                        Estado <span style="color: #ef4444;">*</span>
                        <span style="font-size: 11px; color: #6b7280; font-weight: 400; margin-left: 6px;">(CU-01: Solo instructores activos pueden ser asignados)</span>
                    </label>
                    <?php 
                        $estadoActual = isset($_SESSION['old_input']['inst_estado']) 
                            ? $_SESSION['old_input']['inst_estado'] 
                            : ($registro['inst_estado'] ?? 'Activo');
                    ?>
                    <select name="inst_estado" required
                            style="width: 100%; padding: 12px; border: 1px solid #e5e7eb; border-radius: 8px; font-size: 14px;">
